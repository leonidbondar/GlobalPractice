...
public class ClosePeriod extends User {
	public static void closePeriods() {
		//В результате работы бонусов получилось много открытых периодов, которые нужно закрывать.
		...
		//перед началом каждого цикла зануляем object reference чтобы учесть возможный переход на другую страницу.
		WebElement element = null;
		//пока не могут найти кнопку закрыть на первой и т.д. страницах, то номера страниц должны 
		//увеличиватся
		while (element == null) {
		...
		//потом закрывается период, только одна ссылка Закрыть может быть одновременно
